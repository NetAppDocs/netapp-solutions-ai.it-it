...
----
Nota: - Se non hai configurato sudo senza password sui tuoi nodi, dovrai aggiungere  `--ask-become-pass` a questo comando. Quando si utilizza  `--ask-become-pass`, è fondamentale che l'utente ssh abbia la STESSA password su ogni nodo. - Fare riferimento a  https://github.com/opea-project/Enterprise-RAG/blob/main/deployment/roles/infrastructure/netapp_trident_csi_setup/netapp_trident_integration.md["NetApp Trident CSI Integration per Enterprise RAG"^] per i dettagli. Consultare  https://docs.netapp.com/us-en/trident/trident-install/kubernetes-deploy-operator.html["Documentazione di installazione Trident"^] per ulteriori dettagli.
