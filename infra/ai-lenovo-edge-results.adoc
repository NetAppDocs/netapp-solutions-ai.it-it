...
summary: 'Sono stati eseguiti numerosi test per valutare le prestazioni dell"architettura proposta. Ci sono sei diversi carichi di lavoro come classificazione di immagini, rilevamento di oggetti piccoli, rilevamento di oggetti grandi, imaging medico, speech-to-text ed elaborazione del linguaggio naturale (NLP), che puoi eseguire in tre diversi scenari: offline, single stream e multistream.' 
---
= Risultati dei test
:hardbreaks:
:allow-uri-read: 
