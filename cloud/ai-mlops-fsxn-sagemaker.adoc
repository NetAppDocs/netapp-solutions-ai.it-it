...
Il processo di lettura dei dati prevede la configurazione di FSx ONTAP come bucket S3 privato. Per conoscere le istruzioni di configurazione dettagliate, consulta link:ai-mlops-fsxn-s3.html["Parte 1 - Integrazione di Amazon FSx for NetApp ONTAP (FSx ONTAP) come bucket S3 privato in AWS SageMaker"]
